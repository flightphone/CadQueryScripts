...
def get_cycloid_points(num_points=10000):
    # r подбираем так, чтобы один период (2*pi*r) был равен ширине 2.0
    r = 0.2 / np.pi 
    t = np.linspace(0, 10 * np.pi, num_points)
    
    # Центрируем: вычитаем 1.0 из x, чтобы начать от левого края (-1)
    # Опускаем по y на r, чтобы арка была видна целиком
    x = r * (t - np.sin(t)) - 1.0
    y = r * (1 - np.cos(t)) - 0.5 
...
def generate_map(res=1024):
    ...
    # 3. Отрисовка (белый фон, красная линия)
    ...
    # Чем меньше 'falloff', тем шире и мягче будет ореол свечения
    #falloff = 0.1 
    #glow = np.exp(-dists / falloff) 

    # Усиливаем саму линию в центре (чтобы был яркий стержень)
    #core = smoothstep(0.003, 0, dists)
